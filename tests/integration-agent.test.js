...
describe('Integration Tests', () => {
  test('should create a complete workflow', () => {
    // This would test the entire agent workflow
    // For now, just verify the main components work together
    const agent = new IntegrationAgent('Test goal', './test.har', './test.json', true);
    const dagManager = new DAGManager();
    
    expect(agent).toBeDefined();
    expect(dagManager).toBeDefined();
    
    // Test that they can work together
    const mockRequests = [
      new Request('GET', 'https://api.example.com/auth', {}),
      new Request('POST', 'https://api.example.com/action', {})
    ];
    
    const dag = dagManager.buildFromRequests(mockRequests, 'Mock analysis');
